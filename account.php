...
if(isset($_SESSION['userid'])){
	...
	$title='Quit Your Jobs account details at '.$getbrandrow['name'];
...
<div id="main-content" style="margin-left: 0px; margin-bottom: 0px;">
               ...
               <div class="row bg-title bredcrums-menu">
                ...
                <div class="col-lg-9 col-sm-8 col-md-8 col-xs-12">
                    <ol class="breadcrumb">
                        ...
                        <li class="active breadcrumb-active">Account Details</li>
                    </ol>
                </div>
                <!-- /.col-lg-12 -->
            </div>
		...
		switch ($getbrandrow['api_type']) {
			...
			?>
			<form action="<?php echo $getbrandrow['autologin_url'].$getbrandrow['profile_url']; ?>" method="post" name="loginNow" id="loginNow" autocomplete="off" target="myFrame">
			...
			<input type="hidden" value="<?php echo $getbrandrow['deposit_url']; ?>" name="r">
		        <input type="hidden" name="redirect" value="<?php echo $getbrandrow['profile_url']; ?>">    
			</form>
			...
			<input type="hidden" name="redirect" value="<?php echo $getbrandrow['profile_url']; ?>">
			<input type="hidden" name="extRedir" value="<?php echo $getbrandrow['profile_url']; ?>">
			</form>
			<div class="col-lg-12 col-md-12 col-sm-12 iframe-container">
		  ...
		  case "SPOT-BDB":
			...
			if($getcbrandrow['reg']=='1'){ 
			    $platformurl='https://eu.bancdebinary.com/?rs=1&action=enterAccount&goto='.$getbrandrow['profile_url'].'&customerId='.$customerId.'&uKey='.$authKey.'&lang=en';
			}else{
			    $platformurl='https://bancdebinary.com/?rs=1&action=enterAccount&goto='.$getbrandrow['profile_url'].'&customerId='.$customerId.'&uKey='.$authKey.'&lang=en';
			}
		  ...
		  case "SPOT-USA":		
			...
			echo '<div class="col-lg-12 col-md-12 col-sm-12 iframe-container"><iframe id="iframe" name="myFrame" style="" scrolling="yes" src="'.$getbrandrow['autologin_url'].$getuserrow['session_id'].'&extRedir='.$getbrandrow['profile_url'].'"></iframe></div>';
	 		break;
		  ...
		  case "OPTECK":
			...
			echo '<div class="col-lg-12 col-md-12 col-sm-12 iframe-container"><iframe id="iframe" name="myFrame" scrolling="yes" style="" src="'.$url.'"></iframe></div>';
		       break;
		  ...
		  case "MARKETPULSE":
			...
			echo '<div class="col-lg-12 col-md-12 col-sm-12 iframe-container"><iframe id="iframe" name="myFrame" style="" scrolling="yes" src="'.$getbrandrow['autologin_url'].$token.'&redirectUrl='.$getbrandrow['profile_url'].'?ShowMessage=True"></iframe></div>';
		        break;

		  case "PANDA":		

			echo '<div class="col-lg-12 col-md-12 col-sm-12 iframe-container"><iframe id="iframe" name="myFrame" style="" scrolling="yes" src="'.$getbrandrow['autologin_url'].$getuserrow['email'].'&password='.$getuserrow['password'].'&url='.$getbrandrow['profile_url'].'"></iframe></div>';
	 		break;


		  case "TRADOLOGIC":		

			echo '<div class="col-lg-12 col-md-12 col-sm-12 iframe-container"><iframe id="iframe" name="myFrame" style="" scrolling="yes" src="'.$getbrandrow['profile_url'].$getuserrow['session_id'].'"></iframe></div>';
	 		break;
			
			case "AIRSOFT":
				$url = $getbrandrow['autologin_url'] . '&key=' . $getbrandrow['password'] . '&client_id=' . $getuserrow['spotid'] .
					'&redirect_url=' . $getbrandrow['profile_url'];
				$curl = curl_init($url); 
				curl_setopt($curl, CURLOPT_SSL_VERIFYPEER, true);					
				curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);                                                                      					
				$result = curl_exec($curl);
