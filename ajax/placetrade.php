...
//MODULE=Position&COMMAND=add&customerId=b3adc491-8d35-4542-98d6-135c62593719&type=DigitalCall&amount=2500&endDate=1462299122&assetId=EURUSD&optionId=654a473b-d4e3-48af-b11d-1bdaab08a48f
        ...
        case 'AIRSOFT':
            require_once(__DIR__ . "/../php/Brand/Type/Airsoft.php");
            $airsoft = new Airsoft();
            $aBinaryOptions = $airsoft->setBinaryPositionUrl($amount, $action, $optionId)->request()->getResponse();
            if (!empty($aBinaryOptions['open_positions'])) {
                $count = count($aBinaryOptions['open_positions']);
                if ($aBinaryOptions['open_positions'][$count - 1]['asset_id'] == $optionId) {
                    echo '<h4 class="tradeplaced">TRADE PLACED</h4>';
                } else {
                    echo '<h4 class="tradeplaced">TRADE FAILED! TRY AGAIN</h4>';
                }
