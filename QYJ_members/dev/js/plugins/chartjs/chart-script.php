...
*/
<?php
$call_trades = $db->get_var("select count(*) from trades where status='EXECUTED' and type='CALL'");
$put_trades = $db->get_var("select count(*) from trades where status='EXECUTED' and type='PUT'");
?>
var doughnutData = [
	{
		value: <?= $call_trades; ?>,
...
$c = 0;
foreach($assets as $a) {
	$assetsArr[$c]['name'] = $a->name;
	$assetsArr[$c]['number'] = rand(200, 1000);
	$c++;
}
...
*/
var radarChartData = {
	labels: ["<?= $assetsArr[0]['name']; ?>", "<?= $assetsArr[1]['name']; ?>", "<?= $assetsArr[2]['name']; ?>", "<?= $assetsArr[3]['name']; ?>", "<?= $assetsArr[4]['name']; ?>", "<?= $assetsArr[5]['name']; ?>"],
	datasets: [
		{
			label: "First dataset",
			...
			data: [<?= $assetsArr[0]['number']; ?>,<?= $assetsArr[1]['number']; ?>,<?= $assetsArr[2]['number']; ?>,<?= $assetsArr[3]['number']; ?>,<?= $assetsArr[4]['number']; ?>,<?= $assetsArr[5]['number']; ?>]
		}
	],
};
...
*/
var lineChartData = {
	labels : ["USD/JPY","AUD/USD","EUR/JPY","EUR/USD","GBP/USD","USD/CHF"],
	datasets : [
		{
			label: "My dataset",
