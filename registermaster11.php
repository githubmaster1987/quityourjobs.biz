...
//echo $countryiso;
 ...
 $countryiso = $getcbrandrow['iso'];
  $phoneArray = explode(' ', $phone);
   $url_param = !empty($param) ? '?param='.$param : '';
 if (!empty($phoneArray[1]) && !preg_match("/^[0-9]{4,15}$/", $phoneArray[1])) {
   $_SESSION['registration_failure_message'] = 'Please enter a valid phone number';
   header('Location: f_register_b.php'.$url_param);
   return;
 }
 $phone='+'.$getcbrandrow['phonecode'].$phoneArray[1]; 
